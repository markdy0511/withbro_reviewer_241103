    ...
    if st.session_state.condition_set["commerce_or_not"] == "비커머스":
        ...
        with ch_ranking:
            ...
            if st.session_state.ch_ranking_result is None:
                ...
                ch_summary_statement = ch_ranking_writer.ch_writer(ch_overview_st_dic_summary,st.session_state.overview_result['overview_df'].columns.to_list())
                ...
                for channel in sort_order:
                    ...
                    with st.expander("전체 지표 별 변화 문구"):
                        bullet_output.print_dic_bullet(ch_overview_st_dic[channel])

                    bullet_output.print_dic_bullet(ch_overview_st_dic_summary[channel])
                ...
                for channel in st.session_state.ch_ranking_result["sort_order"]:
                    ...
                    with st.expander("전체 지표 별 변화 문구"):
                        bullet_output.print_dic_bullet(st.session_state.ch_ranking_result["ch_overview_st_dic"][channel])
                    bullet_output.print_dic_bullet(st.session_state.ch_ranking_result["ch_overview_st_dic_summary"][channel])
        ...
        with brnch_ranking:
            ...
            if st.session_state.df_set['used_media']['소재구분'].isnull().all():
                ...
                if st.session_state.brnch_ranking_result is None:
                    ...
                    brnch_summary_statement = ch_ranking_writer.ch_writer(brnch_overview_st_dic_summary,st.session_state.overview_result['overview_df'].columns.to_list())
                    ...
                    for brnch in sort_order:
                        ...
                        with st.expander("전체 지표 별 변화 문구"):
                            bullet_output.print_dic_bullet(brnch_overview_st_dic[brnch])

                        bullet_output.print_dic_bullet(brnch_overview_st_dic_summary[brnch])
        ...
        with overview:
            if st.session_state.overview_result is None:
                ...
                with st.spinner('데이터 분석 중...'):
                    rounded_overview_df = overview_writer.overview_df(st.session_state.df_set['used_media'], st.session_state.df_set['used_ga'], st.session_state.metric_set, st.session_state.trans_metric_set, grouping_period, st.session_state.condition_set, st.session_state.period_set)
                    overview_statement, overview_statement_summary = overview_writer.writer(rounded_overview_df, rounded_overview_df.columns.tolist())
            ...
            if st.session_state.ch_ranking_result is None:
                ...
                with st.spinner('데이터 분석 중...'):
                    for channel in channels:
                        ...
                        overview_ch_statement, overview_ch_statement_summary = overview_writer.writer(rounded_overview_ch_df, rounded_overview_ch_df.columns.tolist())
        ...
        with brnch_ranking:
            ...
            if st.session_state.df_set['used_media']['소재구분'].isnull().all():
                ...
                if st.session_state.brnch_ranking_result is None:
                    ...
                    brnch_summary_statement = ch_ranking_writer.ch_writer(brnch_overview_st_dic,st.session_state.overview_result['overview_df'].columns.to_list())
            ...
            if 'description_cmp_detail' in st.session_state.cmp_ranking_result.keys():
                ...
                if len(filtered_detail_grp_df) == 0:
                    ...
                    try:
                        ...
                        st.session_state.grp_ranking_result['description_grp_detail'] = "데이터 정합성을 확인해주세요."
                        st.write("데이터 정합성을 확인해주세요.")
            else:
                st.write("캠페인 분석 탭을 먼저 실행해주세요.")
